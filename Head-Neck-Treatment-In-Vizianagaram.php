<?php
$meta_title = "Head & Neck Surgery in Vizianagaram | Apple Dental Specialities";
$meta_description = "Specialized major head and neck surgeries in Vizianagaram performed by experienced oral & maxillofacial surgeons. Consult our specialists today.";
include 'header.php';
?>


<section class="dsdl-hero text-center" style="position: relative; height: 40vh; background-image: url('images1/about-bg.jpg'); background-size: cover; background-position: center; display: flex; align-items: center; justify-content: center;">
    <div style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0, 0, 0, 0.4);"></div>
    <h1 style="position: relative; z-index: 2; color: white; font-size: 2.5rem; font-weight: bold; text-transform: uppercase;">Major Head & Neck Surgeries</h1>
</section>


<!-- <section class="mt-5">
<div class="container">
<h2>Head & Neck Surgery in Vizianagaram</h2>
    <p>Head & Neck Surgery in Vizianagaram are performed to treat conditions such as tumors, infections, congenital defects, and trauma. These procedures restore function, improve aesthetics, and enhance the patient's quality of life. Advanced surgical techniques ensure precision and better recovery outcomes.


</p>
</div>
</section> -->




<!-- ========================= 
     SERVICE NEW FIRST SECTION 
     HEAD & NECK SURGERY
========================= --> 
<section class="service_new_first_section"> 
 
    <!-- Decorative background elements --> 
    <div class="service_new_first_section_blob service_new_first_section_blob_one"></div> 
    <div class="service_new_first_section_blob service_new_first_section_blob_two"></div> 
 
 
    <div class="container-fluid service_new_first_section_container"> 
 
        <div class="row align-items-center service_new_first_section_row"> 
 
 
            <!-- ================= LEFT CONTENT ================= --> 
            <div class="col-xl-6 col-lg-6 col-md-12"> 
 
                <div class="service_new_first_section_content"> 
 
 
                    <!-- Small Heading --> 
                    <div class="service_new_first_section_top_title"> 
 
                        <span>HEAD &amp; NECK SURGERY</span> 
 
                        <div class="service_new_first_section_title_line"></div> 
 
                    </div> 
 
 
                    <!-- Main Heading --> 
                    <h1 class="service_new_first_section_heading"> 
 
                        Head &amp; Neck Surgery
                        <br> 
 
                        in 
                        <span>Vizianagaram</span> 
 
                    </h1> 
 
 
                    <!-- Red underline --> 
                    <div class="service_new_first_section_red_line"></div> 
 
 
                    <!-- Description --> 
                    <p class="service_new_first_section_description"> 
 
                        Head &amp; Neck Surgery in Vizianagaram is performed to treat 
                        conditions such as tumors, infections, congenital defects, and 
                        trauma. These procedures restore function, improve aesthetics, 
                        and enhance the patient's quality of life. Advanced surgical 
                        techniques ensure precision and better recovery outcomes.
 
                    </p> 
 
 
                    <!-- ================= BENEFITS ================= --> 
                    <div class="service_new_first_section_benefits"> 
 
 
                        <!-- Benefit 1 --> 
                        <div class="service_new_first_section_benefit"> 
 
                            <div class="service_new_first_section_benefit_icon"> 
 
                                <i class="fa-solid fa-user-doctor"></i> 
 
                            </div> 
 
                            <h6> 
                                Expert Surgical<br> 
                                Care 
                            </h6> 
 
                        </div> 
 
 
                        <!-- Divider --> 
                        <div class="service_new_first_section_benefit_divider"></div> 
 
 
                        <!-- Benefit 2 --> 
                        <div class="service_new_first_section_benefit"> 
 
                            <div class="service_new_first_section_benefit_icon"> 
 
                                <i class="fa-solid fa-stethoscope"></i> 
 
                            </div> 
 
                            <h6> 
                                Treats Complex<br> 
                                Conditions 
                            </h6> 
 
                        </div> 
 
 
                        <!-- Divider --> 
                        <div class="service_new_first_section_benefit_divider"></div> 
 
 
                        <!-- Benefit 3 --> 
                        <div class="service_new_first_section_benefit"> 
 
                            <div class="service_new_first_section_benefit_icon"> 
 
                                <i class="fa-solid fa-arrows-rotate"></i> 
 
                            </div> 
 
                            <h6> 
                                Restores<br> 
                                Function 
                            </h6> 
 
                        </div> 
 
 
                        <!-- Divider --> 
                        <div class="service_new_first_section_benefit_divider"></div> 
 
 
                        <!-- Benefit 4 --> 
                        <div class="service_new_first_section_benefit"> 
 
                            <div class="service_new_first_section_benefit_icon"> 
 
                                <i class="fa-solid fa-heart-pulse"></i> 
 
                            </div> 
 
                            <h6> 
                                Better<br> 
                                Recovery 
                            </h6> 
 
                        </div> 
 
 
                    </div> 
 
                </div> 
 
            </div> 
 
 
            <!-- ================= RIGHT VISUAL ================= --> 
            <div class="col-xl-6 col-lg-6 col-md-12"> 
 
                <div class="service_new_first_section_visual"> 
 
 
                    <!-- Main circular glow --> 
                    <div class="service_new_first_section_visual_ring"></div> 
 
 
                    <!-- Main Head & Neck Surgery Image --> 
                    <div class="service_new_first_section_main_image"> 
 
                        <img 
                            src="assets/images/head-neck-surgery-hero.png" 
                            alt="Head and Neck Surgery in Vizianagaram"
                        > 
 
                    </div> 
 
 
                    <!-- Floating icon 1 --> 
                    <div class="service_new_first_section_floating_icon service_new_first_section_floating_icon_one"> 
 
                        <div class="service_new_first_section_floating_icon_circle"> 
 
                            <i class="fa-solid fa-user-doctor"></i> 
 
                        </div> 
 
                    </div> 
 
 
                    <!-- Floating icon 2 --> 
                    <div class="service_new_first_section_floating_icon service_new_first_section_floating_icon_two"> 
 
                        <div class="service_new_first_section_floating_icon_circle"> 
 
                            <i class="fa-solid fa-stethoscope"></i> 
 
                        </div> 
 
                    </div> 
 
 
                    <!-- Decorative stars --> 
                    <span class="service_new_first_section_star service_new_first_section_star_one"> 
                        ✦ 
                    </span> 
 
                    <span class="service_new_first_section_star service_new_first_section_star_two"> 
                        ✦ 
                    </span> 
 
                    <span class="service_new_first_section_star service_new_first_section_star_three"> 
                        ✦ 
                    </span> 
 
 
                    <!-- Side handwritten text --> 
                    <div class="service_new_first_section_side_text"> 
 
                        <span>Precision</span> 
                        <span>Care</span> 
                        <span>Better Recovery</span> 
 
                    </div> 
 
 
                </div> 
 
            </div> 
 
        </div> 
 
    </div> 
 
 
    <!-- ================= BOTTOM WAVES ================= --> 
    <div class="service_new_first_section_waves"> 
 
        <svg 
            viewBox="0 0 1440 260" 
            preserveAspectRatio="none" 
            xmlns="http://www.w3.org/2000/svg"
        > 
 
            <!-- Light blue wave --> 
            <path 
                class="service_new_first_section_wave_light" 
                d="M0,120 C180,210 320,220 510,165 C700,110 790,90 980,135 C1170,180 1280,220 1440,145 L1440,260 L0,260 Z"> 
            </path> 
 
            <!-- Blue wave --> 
            <path 
                class="service_new_first_section_wave_blue" 
                d="M0,175 C160,250 300,260 500,205 C690,150 820,120 1000,160 C1180,200 1280,235 1440,170 L1440,260 L0,260 Z"> 
            </path> 
 
            <!-- Red accent wave --> 
            <path 
                class="service_new_first_section_wave_red" 
                d="M0,148 C190,230 320,240 515,185 C700,132 820,110 990,150 C1170,190 1290,225 1440,158"> 
            </path> 
 
            <!-- White foreground wave --> 
            <path 
                class="service_new_first_section_wave_white" 
                d="M0,205 C190,270 330,275 530,220 C720,168 830,150 1010,190 C1190,230 1300,255 1440,195 L1440,260 L0,260 Z"> 
            </path> 
 
        </svg> 
 
    </div> 
 
</section>

<!-- <section class="my-5 dental-steps">

<div class="container mt-5">
    <div class="row g-4">
        <div class="col-md-4">
            <div class="card shadow">
                <div class="overflow-hidden">
                    <img src="images1/services/services_img/headneck1.png" class="card-img-top" alt="Image 1">
                </div>
                <div class="card-body text-center">
                    <h5 class="card-title">Step 1:</h5>
                    <p class="card-text"> A thorough diagnosis and imaging tests are conducted to plan the surgery</p>
                </div>
            </div>
        </div>

        <div class="col-md-4">
            <div class="card shadow">
                <div class="overflow-hidden">
                    <img src="images1/services/services_img/headneck2.png" class="card-img-top" alt="Image 2">
                </div>
                <div class="card-body text-center">
                    <h5 class="card-title">Step 2:</h5>
                    <p class="card-text"> The surgery is performed using specialized techniques to treat the condition.
                    </p>
                </div>
            </div>
        </div>

        <div class="col-md-4">
            <div class="card shadow">
                <div class="overflow-hidden">
                    <img src="images1/services/services_img/headneck3.png" class="card-img-top" alt="Image 3">
                </div>
                <div class="card-body text-center">
                    <h5 >Step 3:</h5>
                    <p class="card-text">Post-operative care and rehabilitation help ensure proper healing and recovery</p>
                </div>
            </div>
        </div>
    </div>
</div>


</section> -->


<!-- <section class="my-5 service_bg_page">
    <div class="container my-4 dental-steps">
        Row 1: 1st and 3rd columns with cards, 2nd column is empty
        <div class="row">
            <div class="col-md-4">
                <div class="card">
                    <div class="card-body">
                        <h5 class="card-title"> <strong>Reasons</strong> </h5>
                        <p class="card-text text-left">
                      1)  Treatment of tumors, cysts, or oral cancer affecting the head and neck
                            <br>
                            2)  Correction of structural abnormalities, trauma, or severe infections
                        </p>
                    </div>
                </div>
            </div>
            <div class="col-md-4 empty-column">
                Empty
            </div>
            <div class="col-md-4">
                <div class="card">
                    <div class="card-body">
                        <h5 class="card-title"> <strong>Symptoms</strong></h5>
                        <p class="card-text">1)  Persistent pain, swelling, or lumps in the head, neck, or jaw<br>
                            2) Difficulty swallowing, breathing, or changes in speech 


                        </p>
                    </div>
                </div>
            </div>
        </div>

        Row 2: 1st and 3rd columns are empty, 2nd column contains an image
        <div class="row">
            <div class="col-md-4 empty-column">
                Empty
            </div>
            <div class="col-md-4">
                <div class="card">
                    <img src="images1/services/1.png" class="card-img-top" alt="Image">
                    <img src="images1/services/services_img/s3.png" class="card-img-top" alt="Head & Neck Surgery in Vizianagaram">
                    <div class="card-body">
                    <img src="images1/services/1.png" alt="" class="img-fluid">
                </div>
                </div>
            </div>
            <div class="col-md-4 empty-column">
                Empty
            </div>
        </div>

        Row 3: 1st and 3rd columns with cards, 2nd column is empty
        <div class="row">
            <div class="col-md-4">
                <div class="card">
                    <div class="card-body">
                        <h5 class="card-title"> <strong>Treatment</strong></h5>
                        <p class="card-text">
                            1) Surgical removal of tumors, reconstructive surgery, or biopsy procedures
                            <br>
                            2) Advanced surgical techniques like laser and minimally invasive approaches
                        </p>
                    </div>
                </div>
            </div>
            <div class="col-md-4 empty-column">
                Empty
            </div>
            <div class="col-md-4">
                <div class="card">
                    <div class="card-body">
                        <h5 class="card-title"> <strong>After Treatment</strong> </h5>
                        <p class="card-text">
                            1) Rehabilitation with speech therapy, physiotherapy, or dietary adjustments
                            <br>
                            2)  Regular follow-ups to monitor healing and prevent complications
                        </p>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section> -->





<section class="service_new_second_section">

    <!-- =========================================================
         BACKGROUND DECORATIONS
    ========================================================== -->
    <div class="service_new_second_section_bg_shape service_new_second_section_bg_shape_one"></div>
    <div class="service_new_second_section_bg_shape service_new_second_section_bg_shape_two"></div>
    <div class="service_new_second_section_bg_shape service_new_second_section_bg_shape_three"></div>

    <div class="service_new_second_section_dots service_new_second_section_dots_one"></div>
    <div class="service_new_second_section_dots service_new_second_section_dots_two"></div>


    <!-- LEFT LARGE VERTICAL TEXT -->
    <div class="service_new_second_section_vertical_text">
        HEAD &amp; NECK CARE
    </div>


    <!-- RIGHT HANDWRITTEN NOTE -->
    <div class="service_new_second_section_right_note">

        <span>Expert</span>
        <span>Care</span>
        <span>Better</span>
        <span>Recovery</span>

        <i></i>

    </div>


    <!-- LEFT BOTTOM NOTE -->
    <div class="service_new_second_section_left_note">

        <i class="bi bi-heart"></i>

        <div>
            <span>EXPERT DIAGNOSIS</span>
            <span>PRECISE TREATMENT</span>
            <span>BETTER RECOVERY</span>
        </div>

    </div>


    <!-- RIGHT DECOR IMAGE -->
    <div class="service_new_second_section_mirror">

        <img src="assets/img/services/head-neck-side.png"
             alt="Head and Neck Treatment">

    </div>



    <div class="container-fluid service_new_second_section_container">


        <!-- =====================================================
             HEADER
        ====================================================== -->
        <div class="service_new_second_section_header">

            <div class="service_new_second_section_eyebrow">

                <span></span>

                <p>
                    SPECIALIZED CARE FOR HEAD &amp; NECK CONDITIONS
                </p>

                <span></span>

            </div>


            <h2 class="service_new_second_section_title">

                Head &amp; Neck

                <span>
                    Treatment
                </span>

            </h2>


            <p class="service_new_second_section_subtitle">

                Detailed Diagnosis

                <i></i>

                Personalized Treatment

                <i></i>

                Comprehensive Follow-Up

            </p>

        </div>



        <!-- =====================================================
             PROCESS AREA
        ====================================================== -->
        <div class="service_new_second_section_process">


            <!-- =================================================
                 CENTER CIRCLE
            ================================================== -->
            <div class="service_new_second_section_center">

                <!-- ORBIT CIRCLES -->
                <div class="service_new_second_section_orbit
                            service_new_second_section_orbit_one"></div>

                <div class="service_new_second_section_orbit
                            service_new_second_section_orbit_two"></div>


                <!-- TOP SMALL DECOR -->
                <div class="service_new_second_section_top_mark">

                    <span></span>
                    <i></i>
                    <span></span>

                </div>


                <!-- MAIN CENTER CIRCLE -->
                <div class="service_new_second_section_center_circle">

                    <div class="service_new_second_section_center_image">

                        <img src="assets/img/services/head-neck-process-main.png"
                             alt="Head and Neck Treatment in Vizianagaram">

                    </div>


                    <div class="service_new_second_section_center_note">

                        <span>Expert Care</span>
                        <span>Better Healing</span>

                        <i></i>

                    </div>

                </div>

            </div>



            <!-- =================================================
                 STEP 01
            ================================================== -->
            <article class="service_new_second_section_step
                            service_new_second_section_step_01
                            service_new_second_section_step_blue">

                <span class="service_new_second_section_number">
                    01
                </span>


                <div class="service_new_second_section_step_content">

                    <h3>
                        Consultation &amp; Evaluation
                    </h3>

                    <p>
                        We begin with a detailed consultation to
                        understand your symptoms, medical history
                        and specific head or neck concerns.
                    </p>

                </div>


                <div class="service_new_second_section_step_icon">

                    <img src="assets/img/services/head-neck-step-1.png"
                         alt="Head and Neck Consultation">

                </div>

            </article>



            <!-- =================================================
                 STEP 02
            ================================================== -->
            <article class="service_new_second_section_step
                            service_new_second_section_step_02
                            service_new_second_section_step_red">

                <span class="service_new_second_section_number">
                    02
                </span>


                <div class="service_new_second_section_step_content">

                    <h3>
                        Clinical Examination
                    </h3>

                    <p>
                        The affected area is carefully examined to
                        assess swelling, pain, movement, function
                        and other clinical findings.
                    </p>

                </div>


                <div class="service_new_second_section_step_icon">

                    <img src="assets/img/services/head-neck-step-2.png"
                         alt="Head and Neck Clinical Examination">

                </div>

            </article>



            <!-- =================================================
                 STEP 03
            ================================================== -->
            <article class="service_new_second_section_step
                            service_new_second_section_step_03
                            service_new_second_section_step_blue">

                <span class="service_new_second_section_number">
                    03
                </span>


                <div class="service_new_second_section_step_content">

                    <h3>
                        Imaging &amp; Diagnosis
                    </h3>

                    <p>
                        Appropriate imaging or diagnostic tests may
                        be advised to understand the condition and
                        plan treatment accurately.
                    </p>

                </div>


                <div class="service_new_second_section_step_icon">

                    <img src="assets/img/services/head-neck-step-3.png"
                         alt="Head and Neck Imaging and Diagnosis">

                </div>

            </article>



            <!-- =================================================
                 STEP 04
            ================================================== -->
            <article class="service_new_second_section_step
                            service_new_second_section_step_04
                            service_new_second_section_step_red">

                <span class="service_new_second_section_number">
                    04
                </span>


                <div class="service_new_second_section_step_icon">

                    <img src="assets/img/services/head-neck-step-4.png"
                         alt="Head and Neck Treatment Planning">

                </div>


                <div class="service_new_second_section_step_content">

                    <h3>
                        Personalized Treatment Plan
                    </h3>

                    <p>
                        A treatment plan is created according to
                        the diagnosis, severity of the condition
                        and individual clinical requirements.
                    </p>

                </div>

            </article>



            <!-- =================================================
                 STEP 05
            ================================================== -->
            <article class="service_new_second_section_step
                            service_new_second_section_step_05
                            service_new_second_section_step_blue">

                <div class="service_new_second_section_step_icon">

                    <img src="assets/img/services/head-neck-step-5.png"
                         alt="Head and Neck Treatment Procedure">

                </div>


                <span class="service_new_second_section_number">
                    05
                </span>


                <div class="service_new_second_section_step_content">

                    <h3>
                        Treatment &amp; Procedure
                    </h3>

                    <p>
                        The recommended medical or surgical procedure
                        is performed with careful attention to safety,
                        precision and functional outcomes.
                    </p>

                </div>

            </article>



            <!-- =================================================
                 STEP 06
            ================================================== -->
            <article class="service_new_second_section_step
                            service_new_second_section_step_06
                            service_new_second_section_step_red">

                <div class="service_new_second_section_step_icon">

                    <img src="assets/img/services/head-neck-step-6.png"
                         alt="Head and Neck Recovery Care">

                </div>


                <span class="service_new_second_section_number">
                    06
                </span>


                <div class="service_new_second_section_step_content">

                    <h3>
                        Recovery &amp; Post-Treatment Care
                    </h3>

                    <p>
                        After treatment, appropriate care instructions
                        are provided to support healing, comfort
                        and restoration of normal function.
                    </p>

                </div>

            </article>



            <!-- =================================================
                 STEP 07
            ================================================== -->
            <article class="service_new_second_section_step
                            service_new_second_section_step_07
                            service_new_second_section_step_blue">

                <div class="service_new_second_section_step_icon">

                    <img src="assets/img/services/head-neck-step-7.png"
                         alt="Head and Neck Follow Up Care">

                </div>


                <span class="service_new_second_section_number">
                    07
                </span>


                <div class="service_new_second_section_step_content">

                    <h3>
                        Follow-Up &amp; Monitoring
                    </h3>

                    <p>
                        Regular follow-up visits help monitor healing,
                        assess treatment outcomes and manage any
                        further care that may be required.
                    </p>

                </div>

            </article>


        </div>



        <!-- =====================================================
             BOTTOM FEATURES
        ====================================================== -->
        <div class="service_new_second_section_bottom">


            <div class="service_new_second_section_bottom_item">

                <span class="service_new_second_section_bottom_icon">
                    <i class="bi bi-person-check-fill"></i>
                </span>

                <p>
                    Expert<br>
                    Evaluation
                </p>

            </div>


            <div class="service_new_second_section_bottom_divider"></div>


            <div class="service_new_second_section_bottom_item">

                <span class="service_new_second_section_bottom_icon">
                    <i class="bi bi-clipboard2-pulse-fill"></i>
                </span>

                <p>
                    Detailed<br>
                    Diagnosis
                </p>

            </div>


            <div class="service_new_second_section_bottom_divider"></div>


            <div class="service_new_second_section_bottom_item">

                <span class="service_new_second_section_bottom_icon">
                    <i class="bi bi-shield-check"></i>
                </span>

                <p>
                    Personalized<br>
                    Treatment
                </p>

            </div>


            <div class="service_new_second_section_bottom_divider"></div>


            <div class="service_new_second_section_bottom_item">

                <span class="service_new_second_section_bottom_icon">
                    <i class="bi bi-heart-pulse-fill"></i>
                </span>

                <p>
                    Recovery &amp;<br>
                    Follow-Up Care
                </p>

            </div>


        </div>


    </div>

</section>

<?php include 'footer.php'; ?>