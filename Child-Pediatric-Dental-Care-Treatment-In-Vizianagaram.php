<?php
$meta_title = "Child Dental Care in Vizianagaram | Apple Dental Specialities";
$meta_description = "Gentle, kid-friendly dental care in Vizianagaram for children of all ages. Cavity prevention, checkups & painless treatments. Book now.";
include 'header.php';
?>

 <img src="images1/services/services_img/child-dental-care-bg.png" alt="Contact Us" class="img-fluid">
<!-- <section class="dsdl-hero text-center" style="position: relative; height: 40vh; background-image: url('images1/about-bg.jpg'); background-size: cover; background-position: center; display: flex; align-items: center; justify-content: center;">
    <div style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0, 0, 0, 0.4);"></div>
    <h1 style="position: relative; z-index: 2; color: white; font-size: 2.5rem; font-weight: bold; text-transform: uppercase;">Child Dental Care</h1>
</section> -->


<!-- <section class="mt-5 mx-5">
<div class="container">
<h2>Child Dental Care in Vizianagaram</h2>
    <p>Child Dental Care in Vizianagaram care focuses on maintaining healthy teeth and gums from an early age. Good oral hygiene habits, regular dental check-ups, and a balanced diet help prevent cavities and ensure strong teeth. Early dental care sets the foundation for lifelong oral health.

</p>
</div>
</section> -->






<!-- ========================= 
     SERVICE NEW FIRST SECTION 
     CHILD DENTAL CARE
========================= --> 
<section class="service_new_first_section"> 
 
    <!-- Decorative background elements --> 
    <div class="service_new_first_section_blob service_new_first_section_blob_one"></div> 
    <div class="service_new_first_section_blob service_new_first_section_blob_two"></div> 
 
 
    <div class="container-fluid service_new_first_section_container"> 
 
        <div class="row align-items-center service_new_first_section_row"> 
 
 
            <!-- ================= LEFT CONTENT ================= --> 
            <div class="col-xl-6 col-lg-6 col-md-12"> 
 
                <div class="service_new_first_section_content"> 
 
 
                    <!-- Small Heading --> 
                    <div class="service_new_first_section_top_title"> 
 
                        <span>CHILD DENTAL CARE</span> 
 
                        <div class="service_new_first_section_title_line"></div> 
 
                    </div> 
 
 
                    <!-- Main Heading --> 
                    <h1 class="service_new_first_section_heading"> 
 
                        Child Dental Care 
                        <br> 
 
                        in 
                        <span>Vizianagaram</span> 
 
                    </h1> 
 
 
                    <!-- Red underline --> 
                    <div class="service_new_first_section_red_line"></div> 
 
 
                    <!-- Description --> 
                    <p class="service_new_first_section_description"> 
 
                        Child Dental Care in Vizianagaram focuses on maintaining 
                        healthy teeth and gums from an early age. Good oral hygiene 
                        habits, regular dental check-ups, and a balanced diet help 
                        prevent cavities and ensure strong teeth. Early dental care 
                        sets the foundation for lifelong oral health. 
 
                    </p> 
 
 
                    <!-- ================= BENEFITS ================= --> 
                    <div class="service_new_first_section_benefits"> 
 
 
                        <!-- Benefit 1 --> 
                        <div class="service_new_first_section_benefit"> 
 
                            <div class="service_new_first_section_benefit_icon"> 
 
                                <i class="fa-solid fa-tooth"></i> 
 
                            </div> 
 
                            <h6> 
                                Strong &amp;<br> 
                                Healthy Teeth 
                            </h6> 
 
                        </div> 
 
 
                        <!-- Divider --> 
                        <div class="service_new_first_section_benefit_divider"></div> 
 
 
                        <!-- Benefit 2 --> 
                        <div class="service_new_first_section_benefit"> 
 
                            <div class="service_new_first_section_benefit_icon"> 
 
                                <i class="fa-solid fa-shield-heart"></i> 
 
                            </div> 
 
                            <h6> 
                                Cavity<br> 
                                Prevention 
                            </h6> 
 
                        </div> 
 
 
                        <!-- Divider --> 
                        <div class="service_new_first_section_benefit_divider"></div> 
 
 
                        <!-- Benefit 3 --> 
                        <div class="service_new_first_section_benefit"> 
 
                            <div class="service_new_first_section_benefit_icon"> 
 
                                <i class="fa-solid fa-calendar-check"></i> 
 
                            </div> 
 
                            <h6> 
                                Regular<br> 
                                Check-Ups 
                            </h6> 
 
                        </div> 
 
 
                        <!-- Divider --> 
                        <div class="service_new_first_section_benefit_divider"></div> 
 
 
                        <!-- Benefit 4 --> 
                        <div class="service_new_first_section_benefit"> 
 
                            <div class="service_new_first_section_benefit_icon"> 
 
                                <i class="fa-solid fa-face-smile-beam"></i> 
 
                            </div> 
 
                            <h6> 
                                Healthy<br> 
                                Smile Habits 
                            </h6> 
 
                        </div> 
 
 
                    </div> 
 
                </div> 
 
            </div> 
 
 
            <!-- ================= RIGHT VISUAL ================= --> 
            <div class="col-xl-6 col-lg-6 col-md-12"> 
 
                <div class="service_new_first_section_visual"> 
 
 
                    <!-- Main circular glow --> 
                    <div class="service_new_first_section_visual_ring"></div> 
 
 
                    <!-- Main Child Dental Care Image --> 
                    <div class="service_new_first_section_main_image"> 
 
                        <img 
                            src="assets/images/child-dental-care-hero.png" 
                            alt="Child Dental Care in Vizianagaram"
                        > 
 
                    </div> 
 
 
                    <!-- Floating icon 1 --> 
                    <div class="service_new_first_section_floating_icon service_new_first_section_floating_icon_one"> 
 
                        <div class="service_new_first_section_floating_icon_circle"> 
 
                            <i class="fa-solid fa-tooth"></i> 
 
                        </div> 
 
                    </div> 
 
 
                    <!-- Floating icon 2 --> 
                    <div class="service_new_first_section_floating_icon service_new_first_section_floating_icon_two"> 
 
                        <div class="service_new_first_section_floating_icon_circle"> 
 
                            <i class="fa-solid fa-child-reaching"></i> 
 
                        </div> 
 
                    </div> 
 
 
                    <!-- Decorative stars --> 
                    <span class="service_new_first_section_star service_new_first_section_star_one"> 
                        ✦ 
                    </span> 
 
                    <span class="service_new_first_section_star service_new_first_section_star_two"> 
                        ✦ 
                    </span> 
 
                    <span class="service_new_first_section_star service_new_first_section_star_three"> 
                        ✦ 
                    </span> 
 
 
                    <!-- Side handwritten text --> 
                    <div class="service_new_first_section_side_text"> 
 
                        <span>Healthy</span> 
                        <span>Little</span> 
                        <span>Smiles</span> 
 
                    </div> 
 
 
                </div> 
 
            </div> 
 
        </div> 
 
    </div> 
 
 
    <!-- ================= BOTTOM WAVES ================= --> 
    <div class="service_new_first_section_waves"> 
 
        <svg 
            viewBox="0 0 1440 260" 
            preserveAspectRatio="none" 
            xmlns="http://www.w3.org/2000/svg"
        > 
 
            <!-- Light blue wave --> 
            <path 
                class="service_new_first_section_wave_light" 
                d="M0,120 C180,210 320,220 510,165 C700,110 790,90 980,135 C1170,180 1280,220 1440,145 L1440,260 L0,260 Z"> 
            </path> 
 
 
            <!-- Blue wave --> 
            <path 
                class="service_new_first_section_wave_blue" 
                d="M0,175 C160,250 300,260 500,205 C690,150 820,120 1000,160 C1180,200 1280,235 1440,170 L1440,260 L0,260 Z"> 
            </path> 
 
 
            <!-- Red accent wave --> 
            <path 
                class="service_new_first_section_wave_red" 
                d="M0,148 C190,230 320,240 515,185 C700,132 820,110 990,150 C1170,190 1290,225 1440,158"> 
            </path> 
 
 
            <!-- White foreground wave --> 
            <path 
                class="service_new_first_section_wave_white" 
                d="M0,205 C190,270 330,275 530,220 C720,168 830,150 1010,190 C1190,230 1300,255 1440,195 L1440,260 L0,260 Z"> 
            </path> 
 
        </svg> 
 
    </div> 
 
</section>

<!-- <section class="my-5 dental-steps">
<div class="container ">
    <div class="row g-4">
        <div class="col-md-4">
            <div class="card shadow">
                <div class="overflow-hidden">
                    <img src="images1/services/services_img/child3.png" class="card-img-top" alt="Image 1">
                </div>
                <div class="card-body text-center">
                    <h5 class="card-title">Step 1:</h5>
                    <p class="card-text">  Brush teeth twice daily with fluoride toothpaste to prevent cavities.</p>
                </div>
            </div>
        </div>

        <div class="col-md-4">
            <div class="card shadow">
                <div class="overflow-hidden">
                    <img src="images1/services/services_img/child2.png" class="card-img-top" alt="Image 2">
                </div>
                <div class="card-body text-center">
                    <h5 class="card-title">Step 2:</h5>
                    <p class="card-text"> Encourage flossing and a healthy diet to protect teeth from decay
                    </p>
                </div>
            </div>
        </div>

        <div class="col-md-4">
            <div class="card shadow">
                <div class="overflow-hidden">
                    <img src="images1/services/services_img/child1.png" class="card-img-top" alt="Image 3">
                </div>
                <div class="card-body text-center">
                    <h5 >Step 3:</h5>
                    <p class="card-text">Schedule regular dental visits for check-ups and preventive care.</p>
                </div>
            </div>
        </div>
    </div>
</div>


</section> -->

<!-- <section class="my-5 service_bg_page">
    <div class="container my-4 dental-steps">
        Row 1: 1st and 3rd columns with cards, 2nd column is empty
        <div class="row">
            <div class="col-md-4">
                <div class="card">
                    <div class="card-body">
                        <h5 class="card-title"> <strong>Reasons</strong> </h5>
                        <p class="card-text text-left">
                      1)    Prevents early childhood cavities and dental issues
                            <br>
                            2)Ensures proper growth and alignment of teeth
                        </p>
                    </div>
                </div>
            </div>
            <div class="col-md-4 empty-column">
                Empty
            </div>
            <div class="col-md-4">
                <div class="card">
                    <div class="card-body">
                        <h5 class="card-title"> <strong>Symptoms</strong></h5>
                        <p class="card-text">1)   Tooth decay, sensitivity, or pain in children<br>
                            2) Difficulty chewing or speech development issues


                        </p>
                    </div>
                </div>
            </div>
        </div>

        Row 2: 1st and 3rd columns are empty, 2nd column contains an image
        <div class="row my-5">
            <div class="col-md-4 empty-column">
                Empty
            </div>
            <div class="col-md-4">
                <div class="card">
                    <img src="images1/services/1.png" class="card-img-top" alt="Image">
                    <img src="images1/services/services_img/s1.png" class="card-img-top" alt="Child Dental Care in Vizianagaram">
                    <div class="card-body">
                    <img src="images1/services/1.png" alt="" class="img-fluid">
                </div>
                </div>
            </div>
            <div class="col-md-4 empty-column">
                Empty
            </div>
        </div>

        Row 3: 1st and 3rd columns with cards, 2nd column is empty
        <div class="row">
            <div class="col-md-4">
                <div class="card">
                    <div class="card-body">
                        <h5 class="card-title"> <strong>Treatment</strong></h5>
                        <p class="card-text">
                            1)  Regular dental check-ups, fluoride treatments, and sealants
                            <br>
                            2) Early orthodontic assessment to correct bite or alignment issues
                        </p>
                    </div>
                </div>
            </div>
            <div class="col-md-4 empty-column">
                Empty
            </div>
            <div class="col-md-4">
                <div class="card">
                    <div class="card-body">
                        <h5 class="card-title"> <strong>After Treatment</strong> </h5>
                        <p class="card-text">
                            1) Encourage proper brushing, flossing, and a healthy diet
                            <br>
                            2)  Regular follow-ups to monitor dental growth and oral health
                        </p>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section> -->





<section class="service_new_second_section">

    <!-- =========================================================
         BACKGROUND DECORATIONS
    ========================================================== -->
    <div class="service_new_second_section_bg_shape service_new_second_section_bg_shape_one"></div>
    <div class="service_new_second_section_bg_shape service_new_second_section_bg_shape_two"></div>
    <div class="service_new_second_section_bg_shape service_new_second_section_bg_shape_three"></div>

    <div class="service_new_second_section_dots service_new_second_section_dots_one"></div>
    <div class="service_new_second_section_dots service_new_second_section_dots_two"></div>


    <!-- LEFT LARGE VERTICAL TEXT -->
    <div class="service_new_second_section_vertical_text">
        PEDIATRIC CARE
    </div>


    <!-- RIGHT HANDWRITTEN NOTE -->
    <div class="service_new_second_section_right_note">

        <span>Healthy</span>
        <span>Little</span>
        <span>Smiles</span>
        <span>Grow</span>

        <i></i>

    </div>


    <!-- LEFT BOTTOM NOTE -->
    <div class="service_new_second_section_left_note">

        <i class="bi bi-heart"></i>

        <div>
            <span>GENTLE CARE</span>
            <span>HAPPY SMILES</span>
            <span>HEALTHY FUTURES</span>
        </div>

    </div>


    <!-- RIGHT DENTAL MIRROR -->
    <div class="service_new_second_section_mirror">

        <img src="assets/img/services/pediatric-dental-mirror.png"
             alt="Pediatric Dental Care">

    </div>



    <div class="container-fluid service_new_second_section_container">


        <!-- =====================================================
             HEADER
        ====================================================== -->
        <div class="service_new_second_section_header">

            <div class="service_new_second_section_eyebrow">

                <span></span>

                <p>
                    GENTLE CARE FOR HEALTHY LITTLE SMILES
                </p>

                <span></span>

            </div>


            <h2 class="service_new_second_section_title">

                Pediatric Dental

                <span>
                    Care
                </span>

            </h2>


            <p class="service_new_second_section_subtitle">

                Prevent Dental Problems

                <i></i>

                Build Healthy Habits

                <i></i>

                Protect Growing Smiles

            </p>

        </div>



        <!-- =====================================================
             PROCESS AREA
        ====================================================== -->
        <div class="service_new_second_section_process">


            <!-- =================================================
                 CENTER CIRCLE
            ================================================== -->
            <div class="service_new_second_section_center">

                <!-- ORBIT CIRCLES -->
                <div class="service_new_second_section_orbit
                            service_new_second_section_orbit_one"></div>

                <div class="service_new_second_section_orbit
                            service_new_second_section_orbit_two"></div>


                <!-- TOP SMALL DECOR -->
                <div class="service_new_second_section_top_mark">

                    <span></span>
                    <i></i>
                    <span></span>

                </div>


                <!-- MAIN CENTER CIRCLE -->
                <div class="service_new_second_section_center_circle">

                    <div class="service_new_second_section_center_image">

                        <img src="assets/img/services/pediatric-dental-process-main.png"
                             alt="Pediatric Dental Care Treatment">

                    </div>


                    <div class="service_new_second_section_center_note">

                        <span>Healthy Teeth</span>
                        <span>Happy Childhood</span>

                        <i></i>

                    </div>

                </div>

            </div>



            <!-- =================================================
                 STEP 01
            ================================================== -->
            <article class="service_new_second_section_step
                            service_new_second_section_step_01
                            service_new_second_section_step_blue">

                <span class="service_new_second_section_number">
                    01
                </span>


                <div class="service_new_second_section_step_content">

                    <h3>
                        Child-Friendly Consultation
                    </h3>

                    <p>
                        We begin with a gentle, friendly consultation
                        to help your child feel comfortable and understand
                        their dental needs.
                    </p>

                </div>


                <div class="service_new_second_section_step_icon">

                    <img src="assets/img/services/pediatric-dental-step-1.png"
                         alt="Child Friendly Dental Consultation">

                </div>

            </article>



            <!-- =================================================
                 STEP 02
            ================================================== -->
            <article class="service_new_second_section_step
                            service_new_second_section_step_02
                            service_new_second_section_step_red">

                <span class="service_new_second_section_number">
                    02
                </span>


                <div class="service_new_second_section_step_content">

                    <h3>
                        Dental Examination
                    </h3>

                    <p>
                        The teeth, gums, bite and oral development
                        are carefully examined to identify cavities
                        or other dental concerns.
                    </p>

                </div>


                <div class="service_new_second_section_step_icon">

                    <img src="assets/img/services/pediatric-dental-step-2.png"
                         alt="Pediatric Dental Examination">

                </div>

            </article>



            <!-- =================================================
                 STEP 03
            ================================================== -->
            <article class="service_new_second_section_step
                            service_new_second_section_step_03
                            service_new_second_section_step_blue">

                <span class="service_new_second_section_number">
                    03
                </span>


                <div class="service_new_second_section_step_content">

                    <h3>
                        Preventive Dental Care
                    </h3>

                    <p>
                        Professional cleaning and preventive care
                        help keep teeth healthy and reduce the risk
                        of cavities and gum problems.
                    </p>

                </div>


                <div class="service_new_second_section_step_icon">

                    <img src="assets/img/services/pediatric-dental-step-3.png"
                         alt="Preventive Pediatric Dental Care">

                </div>

            </article>



            <!-- =================================================
                 STEP 04
            ================================================== -->
            <article class="service_new_second_section_step
                            service_new_second_section_step_04
                            service_new_second_section_step_red">

                <span class="service_new_second_section_number">
                    04
                </span>


                <div class="service_new_second_section_step_icon">

                    <img src="assets/img/services/pediatric-dental-step-4.png"
                         alt="Cavity Treatment for Children">

                </div>


                <div class="service_new_second_section_step_content">

                    <h3>
                        Cavity &amp; Tooth Care
                    </h3>

                    <p>
                        Cavities or damaged teeth are treated using
                        gentle techniques designed to protect the
                        child’s tooth and preserve healthy function.
                    </p>

                </div>

            </article>



            <!-- =================================================
                 STEP 05
            ================================================== -->
            <article class="service_new_second_section_step
                            service_new_second_section_step_05
                            service_new_second_section_step_blue">

                <div class="service_new_second_section_step_icon">

                    <img src="assets/img/services/pediatric-dental-step-5.png"
                         alt="Fluoride and Dental Sealants">

                </div>


                <span class="service_new_second_section_number">
                    05
                </span>


                <div class="service_new_second_section_step_content">

                    <h3>
                        Fluoride &amp; Sealants
                    </h3>

                    <p>
                        Fluoride applications and dental sealants
                        may be used to strengthen teeth and provide
                        added protection against cavities.
                    </p>

                </div>

            </article>



            <!-- =================================================
                 STEP 06
            ================================================== -->
            <article class="service_new_second_section_step
                            service_new_second_section_step_06
                            service_new_second_section_step_red">

                <div class="service_new_second_section_step_icon">

                    <img src="assets/img/services/pediatric-dental-step-6.png"
                         alt="Growth and Bite Assessment">

                </div>


                <span class="service_new_second_section_number">
                    06
                </span>


                <div class="service_new_second_section_step_content">

                    <h3>
                        Growth &amp; Bite Assessment
                    </h3>

                    <p>
                        We monitor jaw growth, tooth eruption and
                        bite development to identify orthodontic
                        concerns at an early stage.
                    </p>

                </div>

            </article>



            <!-- =================================================
                 STEP 07
            ================================================== -->
            <article class="service_new_second_section_step
                            service_new_second_section_step_07
                            service_new_second_section_step_blue">

                <div class="service_new_second_section_step_icon">

                    <img src="assets/img/services/pediatric-dental-step-7.png"
                         alt="Pediatric Dental Follow Up">

                </div>


                <span class="service_new_second_section_number">
                    07
                </span>


                <div class="service_new_second_section_step_content">

                    <h3>
                        Follow-Up &amp; Healthy Habits
                    </h3>

                    <p>
                        Regular dental visits, brushing guidance
                        and healthy eating habits help maintain
                        strong teeth as your child grows.
                    </p>

                </div>

            </article>


        </div>



        <!-- =====================================================
             BOTTOM FEATURES
        ====================================================== -->
        <div class="service_new_second_section_bottom">


            <div class="service_new_second_section_bottom_item">

                <span class="service_new_second_section_bottom_icon">
                    <i class="bi bi-shield-check"></i>
                </span>

                <p>
                    Preventive<br>
                    Dental Care
                </p>

            </div>


            <div class="service_new_second_section_bottom_divider"></div>


            <div class="service_new_second_section_bottom_item">

                <span class="service_new_second_section_bottom_icon">
                    <i class="bi bi-people-fill"></i>
                </span>

                <p>
                    Child-Friendly<br>
                    Dental Team
                </p>

            </div>


            <div class="service_new_second_section_bottom_divider"></div>


            <div class="service_new_second_section_bottom_item">

                <span class="service_new_second_section_bottom_icon">
                    <i class="bi bi-heart-pulse-fill"></i>
                </span>

                <p>
                    Gentle &amp;<br>
                    Comfortable Care
                </p>

            </div>


            <div class="service_new_second_section_bottom_divider"></div>


            <div class="service_new_second_section_bottom_item">

                <span class="service_new_second_section_bottom_icon">
                    <i class="bi bi-check-lg"></i>
                </span>

                <p>
                    Healthy Habits<br>
                    For Growing Smiles
                </p>

            </div>


        </div>


    </div>

</section>

<?php include 'footer.php'; ?>