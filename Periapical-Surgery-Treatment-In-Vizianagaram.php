<?php
$meta_title = "Periapical Surgery in Vizianagaram | Apple Dental Specialities";
$meta_description = "Expert periapical (apicoectomy) surgery in Vizianagaram to save infected teeth and treat root-tip infections. Trusted specialists. Book a consultation.";
include 'header.php';
?>



<section class="dsdl-hero text-center" style="position: relative; height: 40vh; background-image: url('images1/about-bg.jpg'); background-size: cover; background-position: center; display: flex; align-items: center; justify-content: center;">
    <div style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0, 0, 0, 0.4);"></div>
    <h1 style="position: relative; z-index: 2; color: white; font-size: 2.5rem; font-weight: bold; text-transform: uppercase;">Periapical Surgery</h1>
</section>


<!-- <section class="mt-5">
    <div class="container">
        <h1>Periapical Surgery Treatment</h1>
        <p>
        <div class="container">
            <h2>Periapical Surgery in Vizianagaram</h2>
            <p>Periapical surgery, also known as an apicoectomy, is a minor surgical procedure to remove infection at the root tip of a tooth. It is performed when a root canal alone cannot fully treat the infection. This procedure helps save the tooth and prevent further complications in Periapical Surgery in Vizianagaram

            </p>
        </div>

        </p>
    </div>



    <div class="container">
        <h1>Periapical Surgery Treatment</h1>
        <p>Periapical surgery, also known as an apicoectomy, is a minor surgical procedure to remove infection at the root tip of a tooth. It is performed when a root canal alone cannot fully treat the infection. This procedure helps save the tooth and prevent further complications.

        </p>
    </div>
</section> -->



<!-- ========================= 
     SERVICE NEW FIRST SECTION 
     PERIAPICAL SURGERY
========================= --> 
<section class="service_new_first_section"> 
 
    <!-- Decorative background elements --> 
    <div class="service_new_first_section_blob service_new_first_section_blob_one"></div> 
    <div class="service_new_first_section_blob service_new_first_section_blob_two"></div> 
 
 
    <div class="container-fluid service_new_first_section_container"> 
 
        <div class="row align-items-center service_new_first_section_row"> 
 
 
            <!-- ================= LEFT CONTENT ================= --> 
            <div class="col-xl-6 col-lg-6 col-md-12"> 
 
                <div class="service_new_first_section_content"> 
 
 
                    <!-- Small Heading --> 
                    <div class="service_new_first_section_top_title"> 
 
                        <span>PERIAPICAL SURGERY</span> 
 
                        <div class="service_new_first_section_title_line"></div> 
 
                    </div> 
 
 
                    <!-- Main Heading --> 
                    <h1 class="service_new_first_section_heading"> 
 
                        Periapical Surgery 
                        <br> 
 
                        in 
                        <span>Vizianagaram</span> 
 
                    </h1> 
 
 
                    <!-- Red underline --> 
                    <div class="service_new_first_section_red_line"></div> 
 
 
                    <!-- Description --> 
                    <p class="service_new_first_section_description"> 
 
                        Periapical surgery, also known as an apicoectomy, is a minor 
                        surgical procedure to remove infection at the root tip of a tooth. 
                        It is performed when a root canal alone cannot fully treat the 
                        infection. This procedure helps save the tooth and prevent further 
                        complications in Periapical Surgery in Vizianagaram.
 
                    </p> 
 
 
                    <!-- ================= BENEFITS ================= --> 
                    <div class="service_new_first_section_benefits"> 
 
 
                        <!-- Benefit 1 --> 
                        <div class="service_new_first_section_benefit"> 
 
                            <div class="service_new_first_section_benefit_icon"> 
 
                                <i class="fa-solid fa-tooth"></i> 
 
                            </div> 
 
                            <h6> 
                                Saves<br> 
                                Natural Tooth 
                            </h6> 
 
                        </div> 
 
 
                        <!-- Divider --> 
                        <div class="service_new_first_section_benefit_divider"></div> 
 
 
                        <!-- Benefit 2 --> 
                        <div class="service_new_first_section_benefit"> 
 
                            <div class="service_new_first_section_benefit_icon"> 
 
                                <i class="fa-solid fa-bacteria"></i> 
 
                            </div> 
 
                            <h6> 
                                Removes Root Tip<br> 
                                Infection 
                            </h6> 
 
                        </div> 
 
 
                        <!-- Divider --> 
                        <div class="service_new_first_section_benefit_divider"></div> 
 
 
                        <!-- Benefit 3 --> 
                        <div class="service_new_first_section_benefit"> 
 
                            <div class="service_new_first_section_benefit_icon"> 
 
                                <i class="fa-solid fa-user-doctor"></i> 
 
                            </div> 
 
                            <h6> 
                                Precise Minor<br> 
                                Surgical Care 
                            </h6> 
 
                        </div> 
 
 
                        <!-- Divider --> 
                        <div class="service_new_first_section_benefit_divider"></div> 
 
 
                        <!-- Benefit 4 --> 
                        <div class="service_new_first_section_benefit"> 
 
                            <div class="service_new_first_section_benefit_icon"> 
 
                                <i class="fa-solid fa-shield-halved"></i> 
 
                            </div> 
 
                            <h6> 
                                Prevents Future<br> 
                                Complications 
                            </h6> 
 
                        </div> 
 
 
                    </div> 
 
                </div> 
 
            </div> 
 
 
            <!-- ================= RIGHT VISUAL ================= --> 
            <div class="col-xl-6 col-lg-6 col-md-12"> 
 
                <div class="service_new_first_section_visual"> 
 
 
                    <!-- Main circular glow --> 
                    <div class="service_new_first_section_visual_ring"></div> 
 
 
                    <!-- Main Periapical Surgery Image --> 
                    <div class="service_new_first_section_main_image"> 
 
                        <img 
                            src="assets/images/periapical-surgery-hero.png" 
                            alt="Periapical Surgery in Vizianagaram"
                        > 
 
                    </div> 
 
 
                    <!-- Floating icon 1 --> 
                    <div class="service_new_first_section_floating_icon service_new_first_section_floating_icon_one"> 
 
                        <div class="service_new_first_section_floating_icon_circle"> 
 
                            <i class="fa-solid fa-tooth"></i> 
 
                        </div> 
 
                    </div> 
 
 
                    <!-- Floating icon 2 --> 
                    <div class="service_new_first_section_floating_icon service_new_first_section_floating_icon_two"> 
 
                        <div class="service_new_first_section_floating_icon_circle"> 
 
                            <i class="fa-solid fa-user-doctor"></i> 
 
                        </div> 
 
                    </div> 
 
 
                    <!-- Decorative stars --> 
                    <span class="service_new_first_section_star service_new_first_section_star_one"> 
                        ✦ 
                    </span> 
 
                    <span class="service_new_first_section_star service_new_first_section_star_two"> 
                        ✦ 
                    </span> 
 
                    <span class="service_new_first_section_star service_new_first_section_star_three"> 
                        ✦ 
                    </span> 
 
 
                    <!-- Side handwritten text --> 
                    <div class="service_new_first_section_side_text"> 
 
                        <span>Save</span> 
                        <span>Your</span> 
                        <span>Tooth</span> 
 
                    </div> 
 
 
                </div> 
 
            </div> 
 
        </div> 
 
    </div> 
 
 
    <!-- ================= BOTTOM WAVES ================= --> 
    <div class="service_new_first_section_waves"> 
 
        <svg 
            viewBox="0 0 1440 260" 
            preserveAspectRatio="none" 
            xmlns="http://www.w3.org/2000/svg"
        > 
 
            <!-- Light blue wave --> 
            <path 
                class="service_new_first_section_wave_light" 
                d="M0,120 C180,210 320,220 510,165 C700,110 790,90 980,135 C1170,180 1280,220 1440,145 L1440,260 L0,260 Z"> 
            </path> 
 
            <!-- Blue wave --> 
            <path 
                class="service_new_first_section_wave_blue" 
                d="M0,175 C160,250 300,260 500,205 C690,150 820,120 1000,160 C1180,200 1280,235 1440,170 L1440,260 L0,260 Z"> 
            </path> 
 
            <!-- Red accent wave --> 
            <path 
                class="service_new_first_section_wave_red" 
                d="M0,148 C190,230 320,240 515,185 C700,132 820,110 990,150 C1170,190 1290,225 1440,158"> 
            </path> 
 
            <!-- White foreground wave --> 
            <path 
                class="service_new_first_section_wave_white" 
                d="M0,205 C190,270 330,275 530,220 C720,168 830,150 1010,190 C1190,230 1300,255 1440,195 L1440,260 L0,260 Z"> 
            </path> 
 
        </svg> 
 
    </div> 
 
</section>

<!-- <section class="my-5 dental-steps">

<div class="container mt-5">
    <div class="row g-4">
        <div class="col-md-4">
            <div class="card shadow">
                <div class="overflow-hidden">
                    <img src="images1/services/services_img/peri1.png" class="card-img-top" alt="Image 1">
                </div>
                <div class="card-body text-center">
                    <h5 class="card-title">Step 1:</h5>
                    <p class="card-text"> The dentist makes a small incision in the gum to access the infected root tip.</p>
                </div>
            </div>
        </div>

        <div class="col-md-4">
            <div class="card shadow">
                <div class="overflow-hidden">
                    <img src="images1/services/services_img/peri2.png" class="card-img-top" alt="Image 2">
                </div>
                <div class="card-body text-center">
                    <h5 class="card-title">Step 2:</h5>
                    <p class="card-text"> The infected tissue and root tip are carefully removed, and the area is cleaned.  
                    </p>
                </div>
            </div>
        </div>

        <div class="col-md-4">
            <div class="card shadow">
                <div class="overflow-hidden">
                    <img src="images1/services/services_img/peri3.png" class="card-img-top" alt="Image 3">
                </div>
                <div class="card-body text-center">
                    <h5 >Step 3:</h5>
                    <p class="card-text">The root end is sealed with a filling, and the gum is sutured for proper healing.</p>
                </div>
            </div>
        </div>
    </div>
</div>


</section> -->

<section class="my-5 service_bg_page">
    <div class="container my-4 dental-steps">
        <!-- Row 1: 1st and 3rd columns with cards, 2nd column is empty -->
        <div class="row">
            <div class="col-md-4">
                <div class="card">
                    <div class="card-body">
                        <h5 class="card-title"> <strong>Reasons</strong> </h5>
                        <p class="card-text text-left">
                            1) Persistent infection or abscess at the root tip
                            <br>
                            2) Failed root canal treatment needing further care
                        </p>
                    </div>

                    <!-- <div class="card-body">
                        <h5 class="card-title"> <strong>Reasons</strong> </h5>
                        <p class="card-text text-left">
                            Persistent infection or abscess at the root tip of a tooth
                            <br>
                            2) Failed root canal treatment requiring further intervention
                        </p>
                    </div> -->





                </div>
            </div>
            <div class="col-md-4 empty-column">
                <!-- Empty -->
            </div>
            <div class="col-md-4">
                <div class="card">
                    <div class="card-body">
                        <h5 class="card-title"> <strong>Symptoms</strong></h5>
                        <p class="card-text">1)Swelling or tenderness near the affected tooth<br>
                            2)Pain that continues even after root canal treatment


                        </p>
                    </div>

                    <!-- <div class="card-body">
                        <h5 class="card-title"> <strong>Symptoms</strong></h5>
                        <p class="card-text">1)Swelling or tenderness near the affected tooth<br>
                            2) Recurring pain even after root canal treatment


                        </p>
                    </div> -->







                </div>
            </div>
        </div>

        <!-- Row 2: 1st and 3rd columns are empty, 2nd column contains an image -->
        <div class="row">
            <div class="col-md-4 empty-column">
                <!-- Empty -->
            </div>
            <div class="col-md-4">
                <div class="card">
                    <!-- <img src="images1/services/1.png" class="card-img-top" alt="Image"> -->
                    <img src="images1/services/services_img/s13.png" class="card-img-top" alt="Periapical Surgery in Vizianagaram">
                    <!-- <div class="card-body">
                    <img src="images1/services/1.png" alt="" class="img-fluid">
                </div> -->
                </div>
            </div>
            <div class="col-md-4 empty-column">
                <!-- Empty -->
            </div>
        </div>

        <!-- Row 3: 1st and 3rd columns with cards, 2nd column is empty -->
        <div class="row">
            <div class="col-md-4">
                <div class="card">
                    <div class="card-body">
                        <h5 class="card-title"> <strong>Treatment</strong></h5>
                        <p class="card-text">
                            1)Removal of infected tissue and root tip through surgery
                            <br>
                            2)Sealing the root end to prevent reinfection.
                        </p>
                    </div>


                    <!-- <div class="card-body">
                        <h5 class="card-title"> <strong>Treatment</strong></h5>
                        <p class="card-text">
                            1)Surgical removal of infected tissue and root tip
                            <br>
                            2)Sealing the root end to prevent further infection
                        </p>
                    </div> -->




                </div>
            </div>
            <div class="col-md-4 empty-column">
                <!-- Empty -->
            </div>
            <div class="col-md-4">
                <div class="card">
                    <div class="card-body">
                        <h5 class="card-title"> <strong>After Treatment</strong> </h5>
                        <p class="card-text">
                            1)Take prescribed medicines and manage pain properly
                            <br>
                            2) Maintain good oral hygiene for faster healing
                        </p>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>



<?php include 'footer.php'; ?>